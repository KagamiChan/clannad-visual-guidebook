...
box[0][0].addClick(() => {
  gotopage(15, 'p15_retB')
})
CmtSummery = ''
CmtSelect =
  '{18日或19日，在资料室认识有纪宁}包括18日，在分支{逃课选择睡觉的地方}选择[到那里去睡]，并且在接下来的分支{下课后，走进资料室的女孩}选择[再说几句话]；或者，18日在分支{逃课选择睡觉的地方}选择[还是回教室吧]或者未触发这个分支，并且在19日的分支{来到旧校舍后的去向（一）}选择[去资料室]，以及在接下来的分支{下课后，走进资料室的女孩}选择[再说几句话]。两种情况均满足认识有纪宁的条件，但只可能遇到其中一种。（同公共线路21日注释2{有纪宁线进入}的相关内容）'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[0][1] = Textbox_Condition_C(
  x(4),
  y(yLevel),
  '18日或19日，在资料室认识有纪宁',
  '5',
...
CmtSelect =
  '若20日，在分支{芳野寻求帮手}中选择了[拒绝]或者未触发这个分支，并且21日在分支{迟到后的午间}选择[赶去学生食堂]或者未触发这个分支，则昨日会有一段没有分支的有纪宁剧情。若不满足上述条件，那么昨日不发生有纪宁剧情，今日这段有分支的有纪宁剧情将被昨日的剧情所替代。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
...
CmtSelect =
  '当且仅当认识智代时（即在16日分支{春原挑战智代（一回目）}选择[去]），选项[智代]才会出现，否则为[杏]、[椋]二项选择。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
...
line[line.length] = Link(
  Point(linx(2.5), liny(3)),
  Point(linx(5.5), liny(3)),
  Point(linx(5.5), liny(-2)),
  box[5][1],
)
line[line.length] = Link(
  Point(linx(2.5), liny(3)),
  Point(linx(5.5), liny(3)),
  Point(linx(5.5), liny(-7)),
  box[3][1],
)
...
CmtSummery = ''
CmtSelect =
  '此处{胜平线进入}即21日，在分支{受害者遗忘的简历}选择[查清地址后送给他]，并且在今日上午的分支{课前，牡丹的处理问题}选择[把牡丹丢还给杏]或者未触发这个分支。没有选择相应选项或者没有触发前一分支的情况均属于否；前一分支选择正确但没有触发后一分支的情况属于是。<br>{渚、风子线进入}即21日，在分支{迟到后的午间}选择[看看窗外]。没有选择相应选项或者没有触发分支的情况均属于否。<br>当进入渚、风子线时，无论作何选择，都不能最终达成胜平线GOOD ENDING。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[13][0] = Textbox_Condition_C(
  x(3.5),
  y(yLevel),
  '胜平线进入或渚、风子线进入',
  '8',
