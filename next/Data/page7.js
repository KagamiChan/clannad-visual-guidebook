...
CmtCaution =
  '随着剧情深入，发动智代连击的机会会增加，连击次数也会增加。若连击次数超过以往最大值就会显示NEW RECORD。重新开始游戏时，纪录仍然保存。'
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
...
CmtSummery =
  '椋又被女生们围住要求占卜，而杏在谈到椋的占卜时却表现得有些奇怪，而且要求朋也和春原去试试。'
CmtSelect =
  '选择[胸脯占卜]或者[自己去占卜]，在16日的分支{古河重新创立话剧社}中选择让[她一个人做]，并且如果20日没有进入美佐枝线，当日就有可能触发隐藏剧情。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[9][1] = Textbox_Selection_C(
  x(5),
  y(yLevel),
  '异常灵验的占卜',
  '3',
...
CmtSummery = ''
CmtSelect =
  '此处{琴美线进入}即15日，在分支{面对泪水打转的椋}选择[无视]，分支{庭院中与少女的谈话}选择[再继续谈话]；17日，在分支{走廊上遇到琴美}选择[跟着她去]。没有选择相应选项或者没有触发任一分支的情况均属于否。<br>{17日、和风子说过话}包括风子线已进入时，在分支{隔壁教室里的陌生女孩}选择[向她搭话]，以及风子线未进入时，在分支空教室里的陌生女孩选择向她搭话。只要触发其一，就满足与风子说过话的条件。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
...
CmtSelect =
  '此处分支选项的个数及内容根据之前的剧情选择有所变化：若您已进入琴美线，则会出现[图书馆]、[回教室去]两个选项；若您17日向风子搭话，则会出现[空教室]、[回教室去]两个选项；同时满足上述条件，则出现[图书馆]、[空教室]、[回教室去]三个选项（若上述情况都不满足，不会进入此分支，请注意前一次条件分支的判断）。<br>选择[图书馆]则继续琴美线、退出风子线并且不会认识风子，否则退出琴美线；选择[空教室]并在之后的分支选择[再看一会]就能认识风子；选择[回教室去]则同时退出琴美线和风子线。'
CmtCaution = ''
CmtOption = '-空教室'
CmtEvents = '11111010'
CmtEvntIru =
...
CmtSelect =
  '选择再看一会就能认识风子，若已进入风子线（即17日在分支{隔壁教室里的陌生女孩}选择[向她搭话]）则能继续；选择离开则退出。'
CmtCaution = ''
CmtOption = '-再看一会儿'
CmtEvents = '11111010'
CmtEvntIru =
  '开始风子大师剧情；风子成为3对3篮球赛和业余棒球成员候选，参与64HIT、有纪宁的咒语、体育仓库事件和伪女友事件'
box[19][1] = Textbox_Selection_C(
  x(2),
  y(yLevel),
  '独自雕刻的风子',
  '6',
