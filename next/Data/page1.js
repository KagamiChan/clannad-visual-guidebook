...
box[0][1] = new ButtonBase(
  x(2.5),
  y(yLevel) - YSCALE / 2,
  'text',
  `<span class=hilight0${sizestr}>一般选择项</span>（简称选项）以及<span class=hilight0${sizestr}>事件</span>。绿色<span class=hilight0${sizestr}>角标</span>表示此选项为进入相应角色的线路的必选选项；红色角标表示选择此选项退出相应角色的线路。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
...
line[line.length] = new vmlline(linx(0), liny(0) - YSCALE / 2, 0, 0)
line[line.length] = new vmlline(linx(0), liny(1), 0, 0)
box[1][1] = new ButtonBase(
  x(2.5),
  y(yLevel) - YSCALE,
  'text',
  `<span class=hilight0${sizestr}>一般选择分支</span>（简称分支）以及<span class=hilight0${sizestr}>条件选择性</span>。蓝色的<span class=hilight0${sizestr}>注释标志</span>表示此选项分支有相应注释说明，内容请参见注释栏与之同号的注释。蓝色角标表示条件选择分支中的分支没有触发时的等同路线走向。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
//----------
...
line[line.length] = new vmlline(linx(0), liny(0) - YSCALE / 2, 0, 0)
line[line.length] = new vmlline(linx(0), liny(1), 0, 0)
box[2][1] = new ButtonBase(
  x(2.5),
  y(yLevel),
  'text',
  `<span class=hilight0${sizestr}>条件选择分支</span>（简称条件分支）。用于判断剧情转向导致选择分支产生变化的条件。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
//----------
...
line[line.length] = new vmlline(linx(0), liny(0) - YSCALE / 2, 0, 0)
line[line.length] = new vmlline(linx(0), liny(1), 0, 0)
box[3][1] = new ButtonBase(
  x(2.5),
  y(yLevel) - YSCALE / 2,
  'text',
  `<span class=hilight0${sizestr}>区块提示</span>。由于选择分支复杂导致流程宽度过大。将一部分流程从主流程中提取出来，并在这部分的位置上用区块提示代替。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
//----------
...
line[line.length] = new vmlline(linx(0), liny(0) - YSCALE / 2, 0, 0)
box[4][1] = new ButtonBase(
  x(2.5),
  y(yLevel),
  'text',
  `<span class=hilight0${sizestr}>流程终止提示</span>。由公共剧情完全进入相应角色剧情时，公共流程相应终结的提示；剧情完结的提示，包括Good Ending和Bad Ending；以及事件流程完结回到原流程的提示。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
//----------
...
line[line.length] = new vmlline(
  box[5][1].upcenter.leftx,
  box[5][1].upcenter.topy - YSCALE / 2,
  0,
  0,
)
box[5][2] = new ButtonBase(
  x(2.5),
  y(yLevel),
  'text',
  `<span class=hilight0${sizestr}>日期／剧情段起讫提示</span>。`,
  'btn0414',
  HEADERWIDTH - x(3),
)
...
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[0],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>智代64HIT</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[1],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>有纪宁的咒语/体育仓库事件</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[2],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>3对3篮球赛</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>伪女友事件</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[4],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>风子大师</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[5],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>琴美好感度</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  src_comment_mark_en[6],
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>枪战游戏/业余棒球</span>相关/无关标志。</td></tr>\
<tr><td width=25% align="left">${getImgStr(
  ...
  COMMENTMARKSIZE,
  COMMENTMARKSIZE,
)}</td><td style="line-height:200%;" align="left"><span class=hilight0414${sizestr}>毕业典礼（AFTER STORY）</span>相关/无关标志。</td></tr>\
</table>`
...
CmtSelect =
  '当您再次开始此游戏，并在前面的分支选择[录点什么上去]，紧接着就会出现这个分支。选择[随便说几句]，就能欣赏到“献给挚友春原的说唱”，第二天课前春原会因此而抱怨。'
CmtCaution =
  '“说唱”一共有七个版本内容，当一个版本剧情触发后，将游戏进行到15日中午左右（即听完春原的抱怨后），则下次重新开始游戏时，您就会听到不同版本的“说唱”。七个版本全部触发后，下次重新开始游戏则回到第一个版本，如此循环……'
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[11][0] = Textbox_Selection_C(
  x(2),
  y(yLevel),
  '献给春原的说唱',
  '2',
