...
CmtSummery = ''
CmtSelect =
  '此处{渚线进入}即16日，在分支{古河重新创立话剧社}选择[帮忙]；18日，在分支{琴美的邀请与古河的约定}选择[今天还是和以前一样与渚一起吃]或者未触发这个分支。没有选择相应选项或者没有触发前一分支的情况均属于否；前一分支选择正确但没有触发后一分支的情况属于是。<br>{风子线进入}即16日，在分支{古河重新创立话剧社}选择[帮忙]；18日，在分支{独自雕刻的风子}选择[再看一会儿]。两次选择都正确才满足风子线进入的条件。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[1][0] = Textbox_Condition_C(
  x(2.5),
  y(yLevel),
  '渚线或者风子线进入',
  '1',
...
CmtSummery = ''
CmtSelect =
  '{18日，和琴美一起吃午饭}包括18日，在分支{琴美的邀请与古河的约定}选择[在这里和琴美一起吃]；或者虽然没有出现这个分支，但是发生了和琴美一起吃午饭的剧情内容（18日渚、风子线未进入，即16日在分支{古河重新创立话剧社}选择让[她一个人做]或者未触发这个分支的话，在分支{寻找春原和朋也内心的思索}分别选择[图书馆]和[不能就这样不理她]，就会发生和琴美一起吃午饭的剧情内容）。不符合以上条件的情况均属于否。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[4][2] = Textbox_Condition_C(
  x(4),
  y(yLevel),
  '18日，和琴美一起吃午饭',
  '2',
...
CmtSelect =
  '选择[跟他过去]，并在之后的分支中分别选择[再看一会儿]、[帮忙]，则可以进入柊胜平线，同时退出美佐枝线（若已进入，即18日分支{宿舍走廊遇见美佐枝}选择[向她搭话]）；如果不这样选择，则继续美佐枝线（若已进入），并且无法再进入柊胜平线。<br>攻略渚线时，在第三个分支{芳野寻求帮手中}选择[帮忙]，则进入AFTER STORY后，才有可能得到芳野的“光”。'
CmtCaution = ''
...
CmtSelect =
  '选择[还可以再来吗]，如果21日中午之前没有进入或者退出了渚线或风子线并且退出智代线或有纪宁线，那么您已经完全进入美佐枝线，但21日白天仍处在公共线路，晚上开始美佐枝角色剧情。如果21日下午仍然处在智代线或者有纪宁线中，则都会出现一个分支，选择正确同样可以进入美佐枝线；如果处在渚、风子线中（具体参见22日公共线路攻略），意味着失去了进入美佐枝线的机会。其角色剧情攻略请参见P26。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[18][0] = Textbox_Selection_C(
  x(1),
  y(yLevel),
  '离开美佐枝的房间',
  '5',
...
CmtSummery = ''
CmtSelect =
  '{符合心跳☆高中触发条件}包括16日，在分支{古河重新创立话剧社}中选择[让她一个人做]；18日，在分支{椋的占卜道具}选择[胸脯占卜]或者在分支{异常灵验的占卜}选择[自己去占卜]。没有选择相应选项或者没有触发前一分支的情况均属于否。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[18][1] = Textbox_Condition_C(
  x(5),
  y(yLevel),
  '符合心跳☆高中触发条件',
  '6',
...
CmtSelect =
  '选择[玩玩看]，进入隐藏剧情心跳☆高中。隐藏剧情触发与否以及其中的任何分支选择结果均不影响之后剧情走向。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[20][0] = Textbox_Selection_C(
  x(4),
  y(yLevel),
  '春原的娱乐书',
  '7',
...
CmtSummery = ''
CmtSelect =
  '公共线路中期隐藏剧情。发生于20日上午，触发条件与藤林杏、椋线18日及之前选择有关，所有选项均不影响剧情发展。心跳☆高中相关攻略请参见P24。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[22][0] = Textbox_Option_C(
  x(3),
  y(yLevel),
  '心跳☆高中',
  '8',
