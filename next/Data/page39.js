...
CmtSummery = ''
CmtSelect =
  '事实上，如果21日在分支{迟到后的午间}选择[看看窗外]，则24日会先一步进入渚、风子的剧情，因而无法进入春原兄妹的线路，而无法触发整个伪女友事件。因此，只有不触发21日的{分支迟到后的午间}（在上一条件（15日在分支{回想起中午庭院中的女生}中选择[在意]）的约束下，有两种方法可以避免触发这个分支：16日在分支{古河重新创立话剧社}中选择[让她一个人做]，或者{18日，和琴美一起吃午饭}（具体参见公共线路19日注释2）。即要求21日之前离开渚、风子线），才会出现接下来的分支{考虑古河}。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[6][2] = Textbox_Condition_C(
  x(4),
  y(yLevel),
  '21日,迟到后的午间',
  '2',
...
CmtSummery = ''
CmtSelect =
  '{23日，咒语游戏中遇到渚}即在23日有纪宁的咒语中触发{咒语游戏中遇到渚}的剧情。有纪宁的咒语相关攻略请参见P32。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
box[8][0] = Textbox_Condition_C(
  x(2),
  y(yLevel),
  '23日，咒语游戏中遇到渚',
  '3',
