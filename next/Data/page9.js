...
line[line.length] = Link(
  Point(linx(2), liny(0) - LINEBASEHEIGHT / 2),
  Point(linx(2), liny(0) - LINEBASEHEIGHT / 2),
)
...
CmtSelect =
  '此处只有选择[叫她名字]或仅剩这个选项时候才能通过：若您选择[翻动书页]或[戳她脸蛋]，则一段剧情过后会再次回到这个分支，这时您刚才选择的那个选项会消失，变成二项选择。若您还是不选择[叫她名字]，则会再次回到这里，这时只剩下一个选项，但是不用您再做出选择了（仅剩的选项不会出现）。'
CmtCaution = ''
CmtOption = ''
CmtEvents = ''
CmtEvntIru = ''
...
CmtSelect =
  '若您没有进入渚、风子线的话，选择[还是回教室]吧，则19日还有一段剧情可以触发并由此进入有纪宁线；若已进入渚、风子线，那么这是您进入有纪宁线的唯一机会，选择[到那里去睡]并在之后的分支中选择[再说几句话]，同时19日的剧情降为不含选项且不影响进程（可选择触发）的普通剧情。'
CmtCaution = ''
CmtOption = '-到那里去睡'
CmtEvents = '10000000'
CmtEvntIru = '有纪宁参与64HIT'
box[12][1] = Textbox_Option_C(
  x(4),
  y(yLevel),
  '逃课选择睡觉的地方',
  '11',
