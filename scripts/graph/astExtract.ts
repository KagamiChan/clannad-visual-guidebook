...
export function extractPageIR(
  ...
  for (let stmtIndex = 0; stmtIndex < body.length; stmtIndex++) {
    ...
    if (isExpressionStatement(stmt)) {
      ...
      if (
        isUpdateExpression(expr) &&
        isIdentifier(expr.argument) &&
        expr.argument.name === 'yLevel'
      ) {
      ...
      if (isAssignmentExpression(expr) && expr.operator === '=') {
        ...
        if (leftRef) {
          ...
          if (isCallExpression(expr.right) && isIdentifier(expr.right.callee)) {
            ...
            const text =
              evalString(expr.right.arguments[2] as any, env) ??
        ...
        if (
          ...
          if (
            ...
            if (
              key === 'header' &&
              isCallExpression(expr.right) &&
              isIdentifier(expr.right.callee)
            ) {
          ...
          if (
            ...
            if (isCallExpression(rhs) && isIdentifier(rhs.callee) && rhs.callee.name === 'Link') {
              ...
              const endpoints: Array<
                | { kind: 'node'; id: string }
                | { kind: 'point'; posBySize: Record<SizeSet, PixelPos | undefined> }
              > = []
              ...
              for (const arg of rhs.arguments) {
                ...
                if (
                  ...
                  arg.callee.name === 'Point'
                ) {
                  const posBySize: Record<SizeSet, PixelPos | undefined> = {
                    0: undefined,
                    1: undefined,
                  }
                  for (const sizeSet of [0, 1] as const) {
              ...
              const nodeEndpoints = endpoints.filter((e) => e.kind === 'node') as Array<{
                kind: 'node'
                id: string
              }>
