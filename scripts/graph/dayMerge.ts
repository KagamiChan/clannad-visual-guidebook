...
export function mergePagesToDays(pages: PageIR[], sizeSetForSegmentation: SizeSet = 0): {
  ...
  for (const ir of pages) {
    const dateNodes = ir.nodes.filter(isDateMarker).sort((a, b) => nodeY(a, sizeSetForSegmentation) - nodeY(b, sizeSetForSegmentation));
    ...
    // Sort all nodes by y
    const sortedNodes = [...ir.nodes].sort((a, b) => nodeY(a, sizeSetForSegmentation) - nodeY(b, sizeSetForSegmentation));
